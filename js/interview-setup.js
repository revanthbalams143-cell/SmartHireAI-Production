(() => {
  ...
  try {
    const prefs=JSON.parse(localStorage.getItem('smarthire.settings.v1')||'{}');
    const type=document.getElementById('setupInterviewType'), diff=document.getElementById('setupDifficulty'), dur=document.getElementById('setupDuration');
    if(prefs.defaultInterview && type) type.value=prefs.defaultInterview;
    if(prefs.defaultDifficulty && diff) diff.value=prefs.defaultDifficulty;
    if(prefs.defaultDuration && dur) dur.value=prefs.defaultDuration;
  } catch(_) {}
