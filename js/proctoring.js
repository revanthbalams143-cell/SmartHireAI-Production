(() => {
  ...
  const apiBase = () => window.smartHireApi?.baseUrl || window.API_BASE || ((location.protocol === "http:" || location.protocol === "https:") ? location.origin : "http://localhost:8080");
  ...
  const objectDetectorUrl = () => window.SMART_HIRE_OBJECT_DETECTION_URL || ((location.hostname === "localhost" || location.hostname === "127.0.0.1") ? "http://127.0.0.1:8094" : ((location.protocol === "http:" || location.protocol === "https:") ? location.origin + "/_ai/object-detection" : "http://localhost:8094"));
