(function () {
    ...
    var API_BASE = (window.smartHireApi && window.smartHireApi.baseUrl)
        ...
        : (((location.protocol === "http:" || location.protocol === "https:") ? location.origin : "http://localhost:8080") + "/api/interviews");
    ...
    var AI_API_BASE = (window.smartHireApi && window.smartHireApi.baseUrl)
        ...
        : `${((location.protocol === "http:" || location.protocol === "https:") ? location.origin : "http://localhost:8080")}/api/ai`;

    var BackendEmotionProvider = function () {};
    BackendEmotionProvider.prototype.detect = async function () {
        try {
            ...
            if (response.ok) {
                ...
                return { dominantEmotion: data.dominantEmotion || "Unavailable", confidence: Number(data.confidence || 0), scores: data.scores || {}, provider: data.provider || "unavailable", simulated: Boolean(data.simulated), available: data.available !== false && data.simulated !== true };
            }
        } catch (error) {}
        return { dominantEmotion: "Unavailable", confidence: 0, scores: {}, provider: "unavailable", simulated: false, available: false };
    };

    var BackendEyeContactProvider = function () {};
    BackendEyeContactProvider.prototype.analyze = async function () {
        try {
            ...
            if (response.ok) {
                var data = await response.json();
                return Object.assign({}, data, { provider: data.provider || "unavailable", simulated: Boolean(data.simulated), available: data.available !== false && data.simulated !== true });
            }
        } catch (error) {}
        return { eyeContactPercentage: 0, attentionLevel: "Unavailable", engagementLevel: "Unavailable", provider: "unavailable", available: false, simulated: false, faceCount: -1 };
    };
    ...
    var pushSignalsToSessionStorage = function () {
        var payload = {
            ...
            architecture: {
                ...
                replaceableWith: ["Custom CNN", "DeepFace", "MediaPipe"]
            }
        };
    ...
    var runLiveSignalCycle = async function () {
        ...
        if (!videoActive) {
            setValidation("Camera unavailable: real emotion and eye-contact analysis is unavailable.");
        } else {
            setValidation("");
        }
    ...
    var initLiveSignalArchitecture = function () {
        ...
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            setValidation("Unsupported browser: camera/microphone APIs are not available. Real AI monitoring cannot run.");
            recordNotification("Interview", "Unsupported browser", "Real AI monitoring is unavailable in this browser.");
        }
