(() => {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      if (Array.isArray(history)) {
        const evaluated = history.find(item => Number.isFinite(Number(item?.overallScore)));
        const latestId = evaluated?.interviewId || history.find(item => item?.interviewId)?.interviewId;
        if (latestId) await loadLatestReport(latestId);
      }
    } catch (_) {
      renderHistory([]);
    }
  ...
  $("downloadAnalyticsReport")?.addEventListener("click", async () => {
    ...
    try {
      const interviewId = Number(button.getAttribute('data-interview-id') || localStorage.getItem('smarthire.latestReportInterviewId') || 0);
      if (!interviewId) throw new Error('No completed report is available yet.');
      const response = await fetch(`${apiBase}/api/analytics/interview/${interviewId}/report/pdf`, { headers: authHeaders() });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      ...
      alert(error?.message || "The report could not be downloaded yet. Complete an interview and try again.");
    } finally {
