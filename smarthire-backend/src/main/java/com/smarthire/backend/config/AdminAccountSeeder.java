 ...
 * variables and are disabled by default. Deployment must opt in explicitly with secrets.
 */
@Component
public class AdminAccountSeeder implements ApplicationRunner {
    ...
    public AdminAccountSeeder(UserRepository userRepository,
                               ...
                               @Value("${app.admin.default-email:}") String defaultAdminEmail,
    ...
    public void run(ApplicationArguments args) {
        if (!bootstrapEnabled) {
            log.info("Admin bootstrap is disabled. No default credentials will be created.");
            return;
        }
        if (defaultAdminEmail == null || defaultAdminEmail.isBlank() || defaultAdminPassword == null || defaultAdminPassword.isBlank()) {
            throw new IllegalStateException("ADMIN_BOOTSTRAP_ENABLED=true requires ADMIN_DEFAULT_EMAIL and ADMIN_DEFAULT_PASSWORD.");
        }
        ...
        log.warn("  Password: {}", passwordSet ? "(set from ADMIN_DEFAULT_PASSWORD environment variable)" : "(kept existing password)");
