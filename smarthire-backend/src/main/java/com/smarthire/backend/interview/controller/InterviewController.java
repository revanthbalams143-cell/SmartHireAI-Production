...
public class InterviewController {
    ...
    @PostMapping("/start")
    public ResponseEntity<InterviewResponse> startInterview(@RequestBody InterviewRequest request, Authentication auth) {
        if (!isPrivileged(auth) && (request.getUserId() == null || !request.getUserId().equals(currentUserId(auth)))) {
    ...
    public ResponseEntity<InterviewEvaluationResponse> evaluateInterview(@RequestBody InterviewEvaluationRequest request, Authentication auth) {
        if (request.getInterviewId() != null && !isPrivileged(auth)) {
            Interview interview = interviewService.getInterviewById(request.getInterviewId());
            if (!owns(interview, auth)) return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
        }
    ...
    public ResponseEntity<Interview> saveInterviewSession(@PathVariable Long interviewId,
                                                          ...
                                                          Authentication auth) {
        Interview existing = interviewService.getInterviewById(interviewId);
        if (!isPrivileged(auth) && !owns(existing, auth)) return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
    ...
    public ResponseEntity<CareerRoadmapResponse> generateCareerRoadmap(@PathVariable Long userId,
                                                                        ...
                                                                        Authentication auth) {
        if (!isCandidateSelfOrAdmin(userId, auth)) return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
    ...
    public ResponseEntity<CandidateEnhancementSnapshotResponse> getCandidateEnhancements(@PathVariable Long userId, Authentication auth) {
        if (!isCandidateSelfOrAdmin(userId, auth)) return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Interview> updateInterview(@PathVariable Long id, @RequestBody InterviewRequest request, Authentication auth) {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<String> deleteInterview(@PathVariable Long id, Authentication auth) {
        Interview existing = interviewService.getInterviewById(id);
        if (!isPrivileged(auth) && !owns(existing, auth)) return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
    ...
    private boolean isCandidateSelfOrAdmin(Long userId, Authentication auth) {
        if (auth == null || !auth.isAuthenticated() || userId == null) return false;
