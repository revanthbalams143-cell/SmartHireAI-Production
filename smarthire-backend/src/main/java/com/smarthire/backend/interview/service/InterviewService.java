...
public class InterviewService {
    ...
    public CandidateEnhancementSnapshotResponse getCandidateEnhancements(Long userId) {
        ...
        if (profileCompletion == null) {
            profileCompletion = buildActualProfileCompletion(userId);
        }
    ...
    private CandidateEnhancementSnapshotResponse.ProfileCompletion buildActualProfileCompletion(Long userId) {
        CandidateEnhancementSnapshotResponse.ProfileCompletion profile = new CandidateEnhancementSnapshotResponse.ProfileCompletion();
        User user = userRepository.findById(userId).orElse(null);
        List<Resume> userResumes = resumeRepository.findAll().stream().filter(r -> userId.equals(r.getUserId())).collect(Collectors.toList());
        List<String> checklist = new ArrayList<>();
        List<String> missing = new ArrayList<>();
        int completed = 0;
        if (user != null && user.getName() != null && !user.getName().isBlank()) { completed++; checklist.add("Account name"); } else missing.add("Add your full name");
        if (user != null && user.getEmail() != null && !user.getEmail().isBlank()) { completed++; checklist.add("Email"); } else missing.add("Add a valid email");
        if (!userResumes.isEmpty()) { completed++; checklist.add("Upload and analyze resume"); } else missing.add("Upload a resume");
        if (!userResumes.isEmpty() && userResumes.stream().anyMatch(r -> r.getSkills()!=null && !r.getSkills().isBlank())) { completed++; checklist.add("Primary skills extracted"); } else missing.add("Complete resume skill extraction");
        if (!interviewRepository.findByUserIdOrderByCreatedAtDesc(userId).isEmpty()) { completed++; checklist.add("Start a mock interview"); } else missing.add("Complete one mock interview");
        int total = 5; profile.setCompletionPercentage(Math.round(completed * 100f / total)); profile.setChecklist(checklist); profile.setMissingItems(missing); return profile;
    }
